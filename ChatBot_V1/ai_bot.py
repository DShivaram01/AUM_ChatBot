...
4. If you'd like to change the topic or category, just type:
   ...
   - `change topic` → to switch to another topic
   - `menu` or `change category` → to go back to the category selection
...
All your questions and the chatbot’s responses will be logged in:
📁 `MyDrive/2_AI_Tutor/chatbot_logs.csv`
...
if not os.path.exists(input_file_path):
    print("📁 Please upload `unified_knowledge_base_updated_names.json` from your computer")
    uploaded = files.upload()
    uploaded_filename = list(uploaded.keys())[0]
    shutil.move(uploaded_filename, input_file_path)
    print(f"✅ File moved to Drive: {input_file_path}")
else:
    print(f"📄 Using existing file in Drive: {input_file_path}")
...
print("✅ All outputs saved to Google Drive → 2_AI_Tutor/")
...
def choose_topic(meta_filtered, category):
    ...
    for i, t in enumerate(topics, 1):
        print(f"{i}. {t}")
    user_topic = input("🔍 Choose a topic: ").strip()
...
while True:
    ...
    for i, cat in enumerate(categories, 1):
        print(f"{i}. {cat}")
    user_cat = input("📌 Choose a category (or type 'all'): ").strip().lower()
...
Answer:"""
        ...
        print("\n🧠 Answer:\n" + answer)
        print("📚 Sources:\n" + "\n".join([f"✔️ {c['subtopic']}" for c in top_chunks]))
        print(f"⏱️ Time taken: {round(time.time() - t0, 2)} sec")
        print("-" * 60)
...
def gradio_chatbot(category, topic, query, history):
    ...
    if not filtered:
        return history + [(query, "❌ No data found for selected topic.")], history
    ...
    if local_embeddings.shape[0] == 0:
        return history + [(query, "❌ No embeddings generated.")], history
...
def gradio_chatbot(category, topic, query, history=[]):
    ...
    if not texts:
        return history + [(query, "❌ No data found for this topic.")]
...
with gr.Blocks() as demo:
    ...
    gr.Markdown("""---\n⚙️ Built with MiniLM + Phi-2 + FAISS + Gradio""")
