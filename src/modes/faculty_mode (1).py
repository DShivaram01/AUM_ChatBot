...
Original file is located at
    https://colab.research.google.com/drive/1soIOmhIj8oK-tWxIPNeLx_pdN3sQOgKh
"""
...
# --- Name-specific cleaner (faculty name) ---
...
def build_faculty_bm25(metadata_list):
    """
    ...
    for idx, prof in enumerate(metadata_list):
        ...
        # Build small indexes (optional, more for future meta filtering)
...
def retrieve_faculty_layered(query: str) -> list:
    """
...
def rerank_results(query, chunks):
    ...
    for c in chunks:
        ...
        # Exact or near-exact department phrase in query
        if dept and dept in query_lower:
            score += 2.0
        else:
            # Require at least 2 overlapping tokens to count as a dept match
            overlap = dept_tokens & query_tokens
            if len(overlap) >= 2:
                score += 1.5
            # If only 1 word overlaps (like "science"), don't treat it as a strong dept match.

        # --- Keywords / interests / projects / courses ---
        # You can keep these, but I’d make them a bit weaker than department match.
    ...
    # Keep only those with positive scores
...
def faculty_pipeline(query, history=[], memory=[]):
    ...
    # 1) If the query is ambiguous (him/her/etc), try resolving via memory
    matched_prof = None
    if is_ambiguous_query(query):
        matched_prof = resolve_query_via_memory(query, memory, metadata)

    if matched_prof:
        # Name locked from memory → skip retrieval, just use this one
        print(f"🎯 Ambiguous query resolved via memory: {matched_prof['name']} (skip BM25/FAISS)")
        top_chunks = [matched_prof]
        name_path = True
    else:
        # 2) Otherwise, use layered retrieval: BM25 first, FAISS fallback
        top_chunks = retrieve_faculty_layered(
            query
        )
        name_path = False
